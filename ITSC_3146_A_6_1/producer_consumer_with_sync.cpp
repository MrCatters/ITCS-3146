...
pthread_mutex_t	data_mutex = PTHREAD_MUTEX_INITIALIZER; // mutex for handling data
pthread_mutex_t	console_mutex = PTHREAD_MUTEX_INITIALIZER;  // mutex for printing to console.
pthread_cond_t empty_array_cond = PTHREAD_MUTEX_INITIALIZER; // cond for empty array signal
pthread_cond_t full_array_cond = PTHREAD_MUTEX_INITIALIZER; // cond for full array signal

// Takes orders from customers and stores them in new_orders
// Will wait to put orders in if array is maxed out.
void* takeOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
		...
		// Check if counter indicates array is full
		if (num_new_orders == 4){
			// Wait for signal
			pthread_cond_wait(&full_array_cond, &data_mutex);
		}
    	...
    	pthread_cond_signal(&empty_array_cond);
...
void* processOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
        ...
        if (num_new_orders == 0){
			// Waits for signal
			pthread_cond_wait(&empty_array_cond, &data_mutex);
		}
		
		// Takes the item out of new_orders and update index.
        o_num = new_orders[index].order_num;
	    item = new_orders[index++].item_num;
        --num_new_orders;

		// End of critical region 3
		pthread_cond_signal(&full_array_cond);
		pthread_mutex_unlock(&data_mutex);
